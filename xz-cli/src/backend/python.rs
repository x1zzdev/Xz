...
///
/// `lib` names the shared object the wrapper loads beside itself; it matches
/// the `--out` passed to `xz build --shared` (default `libXz.so`).
pub fn generate_python_bindings(program: &Program, lib: &str) -> String {
